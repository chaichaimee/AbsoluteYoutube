...
class TrimDialog(wx.Dialog):
    def __init__(self, parent, initial_url=""):
        ...
        elif last_format == "wav":
            self.wavRadio.SetValue(True)
        else:
    ...
    def init_ui(self):
        ...
        formatBox = formatSizer.GetStaticBox()
        formatHelper = guiHelper.BoxSizerHelper(self, sizer=formatSizer)
        formatChoiceSizer = wx.BoxSizer(wx.HORIZONTAL)
        self.mp3Radio = wx.RadioButton(formatBox, label="MP3", style=wx.RB_GROUP)
        self.mp4Radio = wx.RadioButton(formatBox, label="MP4")
        self.wavRadio = wx.RadioButton(formatBox, label="WAV")
        formatChoiceSizer.Add(self.mp3Radio, 0, wx.ALL, 5)
        formatChoiceSizer.Add(self.mp4Radio, 0, wx.ALL, 5)
        formatChoiceSizer.Add(self.wavRadio, 0, wx.ALL, 5)
        formatHelper.addItem(formatChoiceSizer)
        mainSizer.Add(formatSizer, 0, wx.EXPAND | wx.ALL, 5)

        qualitySizer = wx.StaticBoxSizer(wx.VERTICAL, self, label=_("Audio Quality"))
        qualityBox = qualitySizer.GetStaticBox()
        qualityHelper = guiHelper.BoxSizerHelper(self, sizer=qualitySizer)
        choices = ["320 kbps", "256 kbps", "192 kbps", "128 kbps"]
        self.qualityCtrl = wx.ComboBox(qualityBox, choices=choices, style=wx.CB_READONLY)
        self.qualityCtrl.SetStringSelection("320 kbps")
        qualityHelper.addItem(self.qualityCtrl)
        ...
        self.wavRadio.Bind(wx.EVT_RADIOBUTTON, self.on_format_change)
    ...
    def on_preview(self, event):
        ...
        if not url or start_seconds < 0 or (self.video_duration_seconds > 0 and start_seconds >= self.video_duration_seconds):
    ...
    def on_download(self, event):
        ...
        file_format = "mp3" if self.mp3Radio.GetValue() else "mp4" if self.mp4Radio.GetValue() else "wav"
        quality_kbps = int(self.qualityCtrl.GetStringSelection().split()[0]) if self.mp3Radio.GetValue() else 0
        ...
        if file_format == "mp3":
            ...
            base_cmd.extend([
                ...
                "--postprocessor-args", "ffmpeg:-c:v copy -c:a aac -b:a 192k -strict -2"
            ])
        ...
        config.conf[sectionName]["TrimLastURL"] = clean_url
        self.Close()
